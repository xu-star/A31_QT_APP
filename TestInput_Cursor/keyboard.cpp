	...
	: QDialog( parent, fl ), _lastFocusedWidget(NULL)
{
    ...
    connect(toolButton_backspace,SIGNAL(pressed()),this,SLOT(onBackspace()));
    connect(toolButton_capslock,SIGNAL(pressed()),this,SLOT(onCapslock()));
    connect(toolButton_hide,SIGNAL(pressed()),this,SLOT(onEsc()));
    connect(toolButton_Enter, SIGNAL(pressed()), this, SLOT(generateEnterChar()));
	//connect(horizontalSlider,SIGNAL(valueChanged(int)),this,SLOT(onHorizontalSliderValueChanged(int)));

    connect( toolButton_znen, SIGNAL( pressed() ), this, SLOT( onModeInput() ) );
    connect(toolButton_NumPad, SIGNAL(pressed()), SLOT(gotoNumPad()));
    connect(toolButton_SymbolPad, SIGNAL(pressed()), SLOT(gotoSymbolPad()));
...
//将toolButton的signal(())连接到signalMapper的slots(map())
void KeyBoard::connectMapper()
{
    ...
    connect(buttons.space(), SIGNAL(pressed()), signalMapper, SLOT(map()));
}
...
//        qDebug()<<"--------------------------------";
    ...
    if (panelPos.y()>280)
    {
        //panelPos.setY(widgetPos.y() - 2 -212);
        panelPos.setY(0);
    }
    else
    {
