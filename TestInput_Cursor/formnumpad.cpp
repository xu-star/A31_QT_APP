    ...
    ui(new Ui::FormNumPad)
{
    ...
    connect(ui->toolButton_Enter, SIGNAL(pressed()), keyBoard(), SLOT(generateEnterChar()));
    connect(ui->toolButton_BackSpace, SIGNAL(pressed()), keyBoard(), SLOT(generateBackSpaceChar()));
    connect(ui->toolButton_Return, SIGNAL(pressed()), SLOT(onPrevPad()));
    connect(ui->toolButton_Symbol, SIGNAL(pressed()), SLOT(gotoSymbol()));
}
